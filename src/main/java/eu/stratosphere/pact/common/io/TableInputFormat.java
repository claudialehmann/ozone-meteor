...
public class TableInputFormat implements InputFormat<PactRecord, TableInputSplit> {
	...
	/**
	 * Returns the hbaseResult.
	 * 
	 * @return the hbaseResult
	 */
	public HBaseResult getHbaseResult() {
		return this.hbaseResult;
	}

	/**
	 * Returns the hbaseKey.
	 * 
	 * @return the hbaseKey
	 */
	public HBaseKey getHbaseKey() {
	 ...
	 * @return
	 */
	private Scan createScanner(Configuration parameters) {
		Scan scan = null;
	 ...
	 */
	private void createTable(Configuration parameters) {
		...
		try {
			setTable(new HTable(this.hConf, tableName));
		} catch (Exception e) {
			LOG.error(StringUtils.stringifyException(e));
		}
	}
